...
inline void generateVariations(std::list<std::string> &list,
                               const std::string &str, std::string &current,
                               const int index) {
...
bool Graph::incrementIfRawEdgeExists(
    const char fromNode, const char toNode,
    const std::list<char> &extraNodes) noexcept {
  for (auto &[existingFromNode, existingToNode, existingExtraNodes,
              existingWeight] : rawEdges) {
    if (existingFromNode == fromNode && existingToNode == toNode &&
...
void Graph::processTransaction(const std::string &str) {
  ...
  for (int startIndex = 0; startIndex < (str.length() - 1); startIndex++) {
    ...
    for (int offsetIndex = startIndex + 1; offsetIndex < str.length();
         ++offsetIndex) {
...
void Graph::processCLM() {
  ...
  for (const auto &[item, frequency] : itemsets) {
    if (item.length() == 1) {
      ...
      const int pos_y = mapNodeToPosition(item[0]);
      const int pos_x = mapNodeToPosition(item[1]) +
                        mapNodeToPosition(item[1]) * getNodesCount();
      ...
      const int pos_x = mapNodeToPosition(toNode) +
                        mapNodeToPosition(toNode) * getNodesCount() +
...
int Graph::getNodesCount() const noexcept {
